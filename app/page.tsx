...
export default function Home() {
  ...
  return <div className="app">
    ...
    {appointmentModal && profile && session && <AppointmentModal organizationId={profile.organization_id} token={session.access_token} onClose={() => setAppointmentModal(false)} onCreated={(item) => { setAppointments(items => [...items, item].sort((a, b) => a.starts_at.localeCompare(b.starts_at))); setAppointmentModal(false); notify("Agendamento criado com sucesso."); }} />}
...
function AppointmentModal({ organizationId, token, onClose, onCreated }: { organizationId: string; token: string; onClose: () => void; onCreated: (item: Appointment) => void }) {
